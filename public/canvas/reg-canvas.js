let regCanvas = document.querySelector(".reg-canvas");

let windowWidth = document.body.scrollWidth;
let windowHeight = document.body.scrollHeight;

regCanvas.width = windowWidth;
regCanvas.height = windowHeight;

let c = regCanvas.getContext("2d");
  ...
  this.update = () => {
    ...
    for (let i = 0; i < particles.length; i++) {
      ...
      if (
        ...
        c.beginPath();
        c.strokeStyle = `rgba(171, 178, 185, ${map(dist, 30, connecting_distance, 0.5, 0)})`;
...
//create circles
let particles = [];
const max_particles = map(windowWidth, 280, 1920, 53, 225);
for (i = 0; i < max_particles; i++) {
  let radius = 1;
  let x = randomNum(radius - connecting_distance, windowWidth - radius + connecting_distance);
  let y = randomNum(radius - connecting_distance, windowHeight - radius + connecting_distance);
